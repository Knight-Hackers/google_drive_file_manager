...
def main():
    ...
    # Download content
    try:
        # Use the correct file_id from either direct arg or metadata
        content_bytes, name = download_file_content(fm, args.file_id or metadata.get('id'))
    except Exception as e:
    ...
    if args.send_to_gemini:
        if content_text is None:
            # If binary, cannot send raw; inform user
            print('File content is binary or could not be decoded as UTF-8; consider exporting to text or downloading a textual export first.')
            return

        # Truncate if too large
        text_to_send = content_text if len(content_text) <= args.max_chars else content_text[:args.max_chars] + "\n\n...[truncated]..."
        print(f'Sending {len(text_to_send)} characters to Gemini (model=gemini-2.5-flash)')
        try:
            out = send_text_to_gemini(text_to_send)
            print('\nGemini response:\n')
            print(out)
        except Exception as e:
